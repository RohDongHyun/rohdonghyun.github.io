...
$$
A + I = \begin{bmatrix} 1 & 1 \\ 1 & 1 \end{bmatrix}, \qquad \tilde{A} = \frac{1}{2}\begin{bmatrix} 1 & 1 \\ 1 & 1 \end{bmatrix}
$$

(각 노드의 degree가 2이므로 $\frac{1}{2}$ 로 정규화된다.) 초기 feature를 다음과 같이 두자.

$$
H^{(0)} = \begin{bmatrix} 1 \\ 0 \end{bmatrix}
$$

1 layer 후에는

$$
H^{(1)} = \tilde{A} H^{(0)} = \frac{1}{2}\begin{bmatrix} 1 & 1 \\ 1 & 1 \end{bmatrix}\begin{bmatrix} 1 \\ 0 \end{bmatrix} = \begin{bmatrix} 0.5 \\ 0.5 \end{bmatrix}
$$

가 되어, 이미 두 노드의 표현이 같아졌다. 2 layer 후에도

$$
H^{(2)} = \tilde{A} H^{(1)} = \begin{bmatrix} 0.5 \\ 0.5 \end{bmatrix}
$$

로 그대로다. 즉 $h_1^{(1)} = h_2^{(1)}$ 이 되어 노드를 구분하던 정보가 사라졌다.

### Over-squashing 예시: 이진 트리

root node $r$ 가 있고 깊이 $K$ 에 leaf node가 있는 이진 트리를 생각하자. 깊이 $K$ 의 leaf 수는 $2^K$ 이고, 각 leaf $u_i$ 가 scalar feature $x_i \in \mathbb{R}$ 를 가진다고 하자. root가 모든 leaf의 정보를 받아야 하는 task, 예를 들어 label이

$$
y_r = \mathbf{1}\left[ \sum_{i=1}^{2^K} x_i > 0 \right]
$$

로 결정되는 경우를 보자. root의 예측은 $2^K$ 개의 leaf feature 전체에 의존한다. 하지만 GNN의 root representation은 고정 차원 $h_r^{(K)} \in \mathbb{R}^d$ 이므로, $2^K$ 차원의 정보 $(x_1, \dots, x_{2^K}) \in \mathbb{R}^{2^K}$ 를 $\mathbb{R}^d$ 로 압축해야 한다. $K$ 가 커질수록 입력 정보량은 지수적으로 늘지만 표현 차원 $d$ 는 보통 고정이라 정보 손실이 커진다.

메시지 전달 관점에서 보자. 단순 mean aggregation을 쓰면

$$
h_v^{(k+1)} = \frac{1}{|\mathcal{N}(v)|} \sum_{u \in \mathcal{N}(v)} h_u^{(k)}
$$

이고, 이진 트리에서 root로 올라오는 정보는 결국 leaf 전체의 평균 형태가 된다.

$$
h_r^{(K)} = \frac{1}{2^K} \sum_{i=1}^{2^K} x_i
$$

이 경우 root는 leaf 전체의 평균만 알 수 있다. 예를 들어 $X = [1, -1, 1, -1]$ 와 $X' = [0, 0, 0, 0]$ 는 둘 다 평균이 0이라 root 표현이 같아진다.

$$
h_r^{(K)}(X) = h_r^{(K)}(X')
$$

하지만 task가 "양수 leaf가 존재하는가?"라면 $X$ 는 yes, $X'$ 는 no로 정답이 다르다. 즉 task에 필요한 정보는 다른데 GNN 표현은 같아져 버린다.
