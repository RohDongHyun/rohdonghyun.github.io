...
- **Over-smoothing**: layer를 거듭할수록 모든 node의 표현이 서로 비슷해져 구분이 불가능해지는 현상. message passing이 반복되면 정보가 그래프 전체로 평균화되기 때문이다. 그래서 GCN류는 보통 2~3 layer만 쌓는다 ([Li et al., 2018, arXiv:1801.07606](https://arxiv.org/abs/1801.07606)).
  - *수식 예시: 2개 노드 그래프*
    ...
    노드 2개가 서로 연결되어 있고 self-loop도 있다고 하자.
...
- **Over-squashing**: 멀리 떨어진 node 정보를 한 node로 전달할 때, 좁은 경로(bottleneck)를 거치며 지수적으로 많은 정보가 고정 크기 벡터에 압축되어 손실되는 현상 ([Alon & Yahav, 2021, arXiv:2006.05205](https://arxiv.org/abs/2006.05205)).
  -   - *수식 예시: 이진 트리*
    ...
        root node $r$가 있고, 깊이 $K$에 leaf node가 있다고 하자. 깊이 $K$의 leaf 수는 $2^K$ 이다. 각 leaf $u_i$가 scalar feature $x_i\in\mathbb{R}$ 를 가진다고 하자.
  
        
  
        이 때, root가 모든 leaf의 정보를 받아야 하는 task를 생각해보자.
    ...
        즉 root의 예측은 $2^K$개의 leaf feature 전체에 의존한다.
  
      
    
  
        하지만 GNN의 root representation은
    ...
        $K$가 커질수록 입력 정보량은 지수적으로 늘지만, 표현 차원 $d$는 보통 고정되어 있으므로 정보 손실이 커집니다.
  
      
    
  
        이 내용을 메시지 전달 관점에서 보자면,
  
        단순 mean aggregation을 쓰면,
    ...
        이진 트리에서 root로 올라오는 정보를 생각해 보자.
  
      
    
  
        leaf feature가 $x_1,x_2,\dots,x_{2^K}$ 라면 root는 결국 평균 형태의 값을 받는다.
    ...
        이 경우 root는 leaf 전체의 평균만 알 수 있다.
  
      
    
  
        예를 들어 다음 두 입력을 비교해 보면,
    ...
        따라서 mean aggregation만 쓰면 root 표현은 같아질 수 있다.
    ...
        즉, task에 필요한 정보는 다르지만 GNN 표현은 같아져 버린다.
