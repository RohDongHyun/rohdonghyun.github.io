...
> **💡 직관:** 두 학파를 가르는 질문은 "무엇을 우연으로 보느냐"다. 빈도주의는 모수를 고정된 진실로 두고 데이터를 우연의 산물로 본다(같은 실험을 무한히 반복하면 데이터가 어떻게 흩어질까?). 베이지안은 정반대로, 이미 관측한 데이터는 고정된 사실이고 모르는 모수가 불확실한 대상이다(데이터를 봤으니 모수에 대한 내 믿음은 어떻게 바뀌나?).
...
> **💡 직관:** 사후분포는 "선입견(prior)을 가지고 출발해, 증거(likelihood)가 말해주는 만큼 생각을 바꾼 결과"다. 증거가 강하면 선입견은 금방 덮이고, 증거가 빈약하면 선입견이 오래 남는다. 학습이란 결국 이 업데이트를 데이터가 들어올 때마다 반복하는 과정이다.
...
> **💡 직관:** 사후평균이 데이터평균과 사전평균의 가중평균이라는 건, 두 정보원이 각자의 정밀도(precision, 분산의 역수)만큼 발언권을 갖는 줄다리기다. 데이터의 발언권은 $n/\sigma^2$이라 표본이 늘수록 커지고, 결국 사전믿음의 발언권 $1/s^2$을 압도한다. "데이터가 적을 땐 선입견에 기대고, 많아지면 데이터에 맡긴다"가 한 줄로 식에 박혀 있는 셈이다.
...
> **💡 직관:** 켤레사전분포의 매력은 "업데이트해도 분포의 종류가 안 바뀐다"는 데 있다. 데이터를 볼 때마다 복잡한 적분으로 사후분포를 다시 구하는 대신, 분포의 모수 몇 개만 갱신하면 끝난다. 뒤의 베타-이항 예시에서 보듯, 사후분포는 단지 $\alpha$에 성공 횟수를, $\beta$에 실패 횟수를 더한 $Beta(\alpha+X, \beta+n-X)$가 된다.
...
예를 들어, $X \sim N(\mu, \sigma^2)$ 인 자료가 있다고 하자. 이때 분산은 알려져 있다고 가정한다. 평균에 대한 특별한 정보(믿음)가 없는경우 평균이 어떤 값을 가지던지 동일한 정도의 정보를 주도록 하고 싶으면 $\mu$가 균일분포 $\text{Unif}(-\infty, \infty)$ 를 따른다고 가정할 수 있다. 다만 평균의 범위가 $(-\infty, \infty)$이므로, 이러한 분포는 적분가능하지 않다.

사전분포가 improper prior이어도 사후분포가 proper prior가 될 수 있다. 다만 항상 성립하는 것은 아니다.
...
&\approx \frac{1}{P(\mathbf{X})}P(C_k)\prod_{j=1}^{N} P(X_j \vert C_k) \quad \text{Indep. (Naive)}
\end{aligned}
$$

> **💡 직관:** 특성들이 독립이라는 가정은 대개 틀렸다(키와 몸무게는 무관하지 않다). 그럼에도 나이브 베이즈가 잘 작동하는 이유는, 분류는 어차피 가장 큰 클래스 하나만 맞히면 되기 때문이다. 추정한 확률값 자체가 부정확해도 클래스 간 순위만 보존되면 정답을 맞힌다. "틀린 가정으로도 옳은 결정"이 가능한 대표적 예다.
