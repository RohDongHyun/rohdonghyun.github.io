---
title: 07. Markov Chain Monte-Carlo
date: 2025-09-20
tags:
  - Statistics
---
...
> **💡 직관:** MCMC의 핵심 발상은 "원하는 분포를 직접 만들 수 없으면, 그 분포를 평형상태로 갖는 랜덤워크를 설계하자"다. 목표 분포가 산맥의 지형이라면, 높은 곳에 더 오래 머무는 규칙으로 산책자를 풀어놓는다. 충분히 오래 걸어다닌 산책자의 발자국 분포는 결국 그 지형(목표 분포)을 닮게 되고, 그 발자국들을 모아 샘플로 쓴다.
...
> **💡 직관:** 채택확률 $\alpha$의 직관은 "더 그럴듯한 곳으로는 항상 가고, 덜 그럴듯한 곳으로도 가끔은 간다"이다. 항상 더 높은 곳만 좇으면 가장 가까운 봉우리에 갇혀 분포 전체를 그리지 못하므로, 일부러 가끔 내려가며 골고루 탐색한다.
>
> 또한 $\alpha$는 $\pi$의 **비율** $\pi(Y)/\pi(X)$만 쓰기 때문에, 정규화 상수(분모의 $f(x)$, 즉 evidence)가 약분돼 사라진다. 베이지안에서 가장 구하기 어려운 게 바로 이 정규화 상수인데, MCMC는 그것을 몰라도 — $\pi$에 비례하는 값만 알아도 — 작동한다는 것이 결정적 장점이다.
...
> **💡 직관:** 깁스 샘플링은 "여러 변수를 한꺼번에 못 뽑으면, 나머지를 다 고정한 채 한 변수씩 번갈아 뽑자"는 전략이다. 다차원 분포 표본 추출이라는 어려운 문제를, 우리가 아는 1차원 조건부 분포들의 연쇄로 쪼갠다. Metropolis-Hastings처럼 후보를 버리는 일 없이 매번 채택하므로(조건부 분포를 직접 알 때) 효율적이지만, 변수들이 강하게 얽혀 있으면 한 축씩만 움직이는 탓에 탐색이 느려질 수 있다.
