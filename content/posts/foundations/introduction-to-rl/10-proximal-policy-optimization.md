---
title: 10. Proximal Policy Optimization
date: 2025-09-22
tags:
...
![](/images/1c58d85f-5014-493d-874c-f4d5bc313daa-image.png)
...
$$
J(\theta, \theta_{\text{old}}) = \mathbb{E}*{\pi*\theta} \left[ \frac{\pi_\theta(a \mid s)}{\pi_{\theta_{\text{old}}}(a \mid s)} A^{\pi_{\theta_{\text{old}}}}(s, a) \right]
$$

$$
\overline{KL}(\theta, \theta_{\text{old}}) = \mathbb{E}*{\pi*\theta} \left[ KL(\pi_{\theta_{\text{old}}}(\cdot \mid s), \pi_\theta(\cdot \mid s)) \right]
$$
...
$$
\max  \mathbb{E}*{\pi*\theta} \left[ \frac{\pi_\theta(a \mid s)}{\pi_{\theta_{\text{old}}}(a \mid s)} A^{\pi_{\theta_{\text{old}}}}(s, a) - \lambda KL(\pi_{\theta_{\text{old}}}(\cdot \mid s), \pi_\theta(\cdot \mid s)) \right]
$$
...
$$
\max  \mathbb{E}*{\pi*\theta} \left[ J_{\text{PPO}} \right]
$$
...
> PPO는 objective 값 자체가 아니라, old policy에서 new policy로 행동 확률이 변하는 값, 즉 new policy가 old policy에서 멀어지는 정도를 제한하고자 한다. (objective 값은 advantage 값 크기에도 영향 받음)
...
- $A_t > 0$이고 $r_t > 1 + \epsilon$: 좋은 행동 확률을 너무 올림 → clip 필요
- $A_t > 0$이고 $r_t > 1 - \epsilon$: 좋은 행동 확률을 너무 낮춤 → clip하면 안 됨
    - 다시 올리는 gradient가 필요
- $A_t < 0$이고 $r_t > 1 - \epsilon$: 나쁜 행동 확률을 너무 낮춤 → clip 필요
- $A_t < 0$이고 $r_t > 1 + \epsilon$: 나쁜 행동 확률을 너무 올림 → clip하면 안 됨
    - 다시 낮추는 gradient가 필요
...
$$
J_{\text{KLPEN}} = \mathbb{E}*{\pi*\theta} \left[ \frac{\pi_\theta(a \mid s)}{\pi_{\theta_{\text{old}}}(a \mid s)} A^{\pi_{\theta_{\text{old}}}}(s, a) - \lambda KL(\pi_{\theta_{\text{old}}}(\cdot \mid s), \pi_\theta(\cdot \mid s)) \right]
$$
