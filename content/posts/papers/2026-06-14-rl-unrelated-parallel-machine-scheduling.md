...
문제는 이 상태(state)를 신경망에 어떻게 입력하느냐다. 작업 수·기계 수가 인스턴스마다 다르기 때문에, 고정 크기 벡터로는 표현하기 어렵다. 그래서 그래프로 표현하고 GNN으로 처리한다. 그래프는 node 수가 달라져도 같은 신경망이 처리할 수 있어, **한 번 학습하면 작업 수·기계 수에 상관없이 재사용**할 수 있다. 이것이 이 논문의 가장 큰 실용적 장점이다.
...
edge는 네 종류로 나뉘며, 각각 다른 관계를 표현한다.

- **$E_t^a$ (같은 작업끼리)**: 양방향. 서로 다른 기계에 걸쳐 있는 **같은 작업 $j$** 의 쌍 node들을 연결. "이 작업은 이 기계들 중 하나에 배정될 후보다"라는 경합 관계. (Figure 2a)
- **$E_t^b$ (같은 기계끼리)**: 방향성 edge. 같은 기계 $k$, 다른 작업들을 연결하며 edge feature로 **셋업시간 $s_{jj'k}$**(작업 $j \to j'$)를 싣는다. (Figure 2b)
- **$E_t^c$ (같은 기계, 역방향)**: $E_t^b$의 역방향. $s_{j'jk}$를 feature로 실어 반대 방향 셋업도 학습에 활용. 방향을 뒤집어 양쪽 정보를 모두 신경망에 흘려준다. (Figure 2b)
- **$E_t^d$ (기계 node ↔ 쌍 node)**: 양방향. 기계 node $v_k^b$와 쌍 node $v_{jk}^a$를 잇고, **기계의 마지막 작업을 고려한 셋업시간**을 feature로 담는다. "이 기계에 지금 이 작업을 올리면 셋업이 얼마나 드는가". (Figure 2c)

핵심은, 셋업시간이라는 핵심 비용이 **node가 아니라 edge feature** 로 자연스럽게 들어간다는 점이다. 이것이 line graph 표현을 택한 이유다.
...
그다음 **GATv2(Graph Attention Network v2)** 를 적용한다. 핵심은 앞서 정의한 네 종류의 edge $E^a, E^b, E^c, E^d$ 각각에 대해 **별도의 서브그래프로 나눠 독립적으로 처리**한 뒤 결과를 합친다는 점이다. 관계 종류마다 의미가 다르므로 따로 학습하는 것이 자연스럽다.
